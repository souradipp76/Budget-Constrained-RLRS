...
def train(CONFIG):
    ...
    if CONFIG.continue_train and os.path.isfile(CONFIG.saved_model_name):
...
def predict(CONFIG, mode = 'test'):
    ...
    if mode == 'test':
        ...
        gen = input_generator(CONFIG.validation_set, CONFIG.batch_size, CONFIG.seq_len, CONFIG.model_type, shuffle = True)
    else:
...
def calc_metrics(mode='train'):
    metric_keys = [ 'p@5', 'p@10', 'map@5', 'map@10', 'map@30' ]
    filename = f"../outputs/{mode}_predict.out"
...
if __name__ == "__main__":
    ...
    if CONFIG.train:
        ...
        # print(batch[0][0].shape, batch[0][1].shape, batch[1].shape, batch[0][1][0][0])
    ...
    valid_preds = predict(CONFIG, 'valid')
    test_preds = predict(CONFIG, 'test')

...
def plot_comparison():
    ...
    # --- All-5-algorithm overlay panels ---
    # Derive budgets from what actually exists in results (not a hardcoded list)
    combined_df = _load_all_results()
    overlay_budgets = sorted(combined_df['user_budget'].unique().tolist())
    sub_combined = combined_df[combined_df.user_budget.isin(overlay_budgets)]

    ncols = max(1, len(overlay_budgets))
    f, axes = plt.subplots(nrows=1, ncols=ncols,
                           figsize=(max(8, 5 * ncols), 6),
                           sharex=True, sharey=True, squeeze=False)
    axes = axes[0]  # unwrap to 1-D array after squeeze=False
    for n, ub in enumerate(overlay_budgets):
        ...
        if n < len(overlay_budgets) - 1:
            leg = axes[n].get_legend()
            if leg is not None:
                leg.remove()
...
def _build_candidate_features(k, rel_cand, cost_cand, budget, cum_rel):
    """Build (n_cand, 5) float32 array for one user-step over the candidate set.

    Using np.column_stack on pre-sliced arrays avoids the full-corpus allocation
    that made the original _build_item_features_tensor slow.
    """
    n = len(rel_cand)
    return np.column_stack([
        np.full(n, k,       dtype=np.float32),
        rel_cand.astype(np.float32),
        cost_cand.astype(np.float32),
        np.full(n, budget,  dtype=np.float32),
        np.full(n, cum_rel, dtype=np.float32),
    ])


def _action_logprob_over_candidates(policy_net, cand_feats_np, chosen_local,
                                     excluded_local_set, device):
    """Exact log-prob of chosen_local within the candidate feature set.

    cand_feats_np : (n_cand, 5) numpy array — already built for this step.
    chosen_local  : index of the chosen action within the candidate array.
    excluded_local_set : set of local indices already placed in the slate.
    """
    feats = torch.from_numpy(cand_feats_np).to(device)
    logits = policy_net(feats)
    n_cand = logits.shape[0]
    mask = torch.ones(n_cand, dtype=torch.bool, device=device)
    for idx in excluded_local_set:
        mask[idx] = False
    logits = logits.masked_fill(~mask, float('-inf'))
    return torch.log_softmax(logits, dim=0)[chosen_local]


def _generate_user_slate_neural(policy_net, all_user_budgets,
                                 rel_cand, cost_cand, candidate_ids,
                                 slate_size, user_ind, device, deterministic=False):
    """Generate one user's slate over the candidate item set.

    Returns
    -------
    chosen_global : list[int]  global item indices forming the slate
    log_probs     : list[float] log π(a_k | s_k) for each step
    state_feats   : list[[k, budget, cum_rel]] for value net
    chosen_local  : list[int]  local indices within candidate_ids
    """
    chosen_global, log_probs, state_feats, chosen_local = [], [], [], []
    ...
    with torch.no_grad():
        for k in range(slate_size):
            arr   = _build_candidate_features(k, rel_cand, cost_cand, budget, cum_rel)
            feats = torch.from_numpy(arr).to(device)
            logits = policy_net(feats)
            ...
            log_p  = torch.log_softmax(logits, dim=0)[local_idx].item()
            action = int(candidate_ids[local_idx])

            log_probs.append(log_p)
            state_feats.append([float(k), budget, cum_rel])
            chosen_local.append(local_idx)
            chosen_global.append(action)
            budget  -= float(cost_cand[local_idx])
            cum_rel += float(np.exp(rel_cand[local_idx]))

    return chosen_global, log_probs, state_feats, chosen_local


def _collect_trajectories(policy_net, all_user_budgets,
                           rel_cand, cost_cand, candidate_ids,
                           slate_size, num_users, device):
    """Collect slates + per-step metadata for all users using the current policy."""
    n_cand = len(candidate_ids)
    ...
    for u in range(num_users):
        glob, lps, sfs, cl = _generate_user_slate_neural(
            policy_net, all_user_budgets,
            rel_cand, cost_cand, candidate_ids,
            slate_size, u, device
        )
        all_slates[u]       = glob
        ...
        all_chosen_local.append(cl)

    return all_slates, all_log_probs, all_state_feats, all_chosen_local
...
def run_ppo(
        ...
        lr=3e-4, entropy_coef=0.01, num_candidates=5000, device=None
):
    """Proximal Policy Optimisation for budget-constrained slate recommendation.

    Scoring is restricted to the top-`num_candidates` items by relevance so that
    the O(num_docs) forward pass that caused the original hang is replaced by an
    O(num_candidates) one (~28x faster with the default 5 000 candidates).
    Training uses proper mini-batch updates over the collected rollout buffer
    rather than a per-step gradient step.
    """
    if device is None:
    ...
    # --- candidate set (fixed across iterations) ---
    candidate_ids = np.argsort(-doc_relevances)[:num_candidates]
    rel_cand  = doc_relevances[candidate_ids].astype(np.float32)
    cost_cand = doc_costs[candidate_ids].astype(np.float32)
    # map global id → local position in candidate_ids
    global_to_local = {int(g): l for l, g in enumerate(candidate_ids)}
    ...
    for it in range(num_iter):
        ...
        all_slates, old_log_probs, all_state_feats, all_chosen_local = \
            _collect_trajectories(
                policy_net, user_initial_budget,
                rel_cand, cost_cand, candidate_ids,
                slate_size, num_users, device
            )
        ...
        sf_batch = torch.FloatTensor(
            [sf for sfs in all_state_feats for sf in sfs]
        ).to(device)                                         # (num_users*slate_size, 3)
        with torch.no_grad():
            vals_flat = value_net(sf_batch).cpu().numpy()
        idx = 0
        for u in range(num_users):
            vals = vals_flat[idx: idx + slate_size].tolist()
            idx += slate_size
        ...
        # Each entry: (cand_feats_np, chosen_local, old_lp, norm_adv, ret, state_feat)
        buffer = []
        for u in range(num_users):
            ...
            ex_local: list[int] = []
            for k in range(slate_size):
                cl  = all_chosen_local[u][k]
                olp = old_log_probs[u][k]
                adv = (all_advantages[u][k] - adv_mean) / adv_std
                ret = all_returns[u][k]
                sf  = all_state_feats[u][k]
                arr = _build_candidate_features(k, rel_cand, cost_cand, budget, cum_rel)
                buffer.append((arr, list(ex_local), cl, olp, adv, ret, sf))
                ex_local.append(cl)
                budget  -= float(cost_cand[cl])
                cum_rel += float(np.exp(rel_cand[cl]))

        # --- mini-batch PPO update ---
        policy_net.train()
        value_net.train()
        indices = np.arange(len(buffer))
        batch_size = max(1, len(buffer) // 10)
        for _ in range(ppo_epochs):
            ...
            for start in range(0, len(buffer), batch_size):
                ...
                for bi in batch_idx:
                    arr, ex_local, cl, olp, adv, ret, sf = buffer[bi]
                    # value loss
                    sf_t  = torch.FloatTensor(sf).unsqueeze(0).to(device)
                    v_pred = value_net(sf_t)
                    val_losses.append((v_pred - ret) ** 2)
                    # policy loss (exact log-prob over candidate set)
                    new_lp = _action_logprob_over_candidates(
                        policy_net, arr, cl, set(ex_local), device
                    )
                    ratio = torch.exp(new_lp - olp)
                    surr  = torch.min(ratio * adv,
                                      torch.clamp(ratio, 1-clip_eps, 1+clip_eps) * adv)
                    # entropy bonus over the full candidate set at this step
                    feats_t = torch.from_numpy(arr).to(device)
                    with torch.no_grad():
                        ent_logits = policy_net(feats_t)
                    ent_p = torch.softmax(ent_logits, dim=0)
                    entropy = -(ent_p * (ent_p + 1e-8).log()).sum()
                    pol_losses.append(-surr - entropy_coef * entropy)

                pol_loss = torch.stack(pol_losses).mean()
                pol_opt.zero_grad()
                pol_loss.backward()
        ...
        print(f"PPO Iteration {it+1}/{num_iter}: "
              f"Play Rate={metrics[0]:.4f}, Avg Imp={metrics[1]:.4f}")
        results.append(metrics)
...
def run_grpo(
        ...
        num_candidates=5000, device=None
):
    ...
    Scoring is restricted to the top-num_candidates items by relevance for speed.
    """
    if device is None:
        device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    np.random.seed(seed)
    ...
    for it in range(num_iter):
        ...
        group_slates, group_log_probs, group_chosen_local, group_rewards = [], [], [], []
        for g in range(G):
            slates, lps, _, chosen_locals = _collect_trajectories(
                policy_net, user_initial_budget,
                rel_cand, cost_cand, candidate_ids,
                slate_size, num_users, device
            )
            ...
            group_chosen_local.append(chosen_locals)
        ...
        # Lightweight rollout buffer: store metadata only, recompute features on the fly
        # Each entry: (g, u, k, budget_at_k, cum_rel_at_k, ex_local_at_k, cl, nr)
        buffer = []
        for g in range(G):
            for u in range(num_users):
                budget  = float(user_initial_budget[u])
                ...
                for k in range(slate_size):
                    cl = group_chosen_local[g][u][k]
                    buffer.append((k, budget, cum_rel, list(ex_local), cl, nr))
                    ex_local.append(cl)
                    budget  -= float(cost_cand[cl])
                    cum_rel += float(np.exp(rel_cand[cl]))

        # Mini-batch GRPO update (features recomputed inside loop — no large buffer)
        policy_net.train()
        indices = np.arange(len(buffer))
        batch_size = max(1, len(buffer) // 10)
        np.random.shuffle(indices)
        for start in range(0, len(buffer), batch_size):
            ...
            pol_losses = []
            for bi in batch_idx:
                k, budget, cum_rel, ex_local, cl, nr = buffer[bi]
                arr = _build_candidate_features(k, rel_cand, cost_cand, budget, cum_rel)
                new_lp = _action_logprob_over_candidates(
                    policy_net, arr, cl, set(ex_local), device
                )
                pol_losses.append(-new_lp * nr)
            pol_loss = torch.stack(pol_losses).mean()
            pol_opt.zero_grad()
            pol_loss.backward()
            torch.nn.utils.clip_grad_norm_(policy_net.parameters(), 0.5)
...
def run_gspo(
        ...
        doc_costs, doc_relevances, discount_factor,
        num_iter=5, seed=0, G=4, lr=3e-4,
        clip_eps=0.2, num_candidates=5000, device=None
):
    """Group Sequence Policy Optimization (GSPO) for budget-constrained slate recommendation.

    Like GRPO but the IS ratio is accumulated at the *sequence* level and clipped
    once per trajectory rather than per step.  No critic needed.
    Uses per-trajectory backward() with gradient accumulation to avoid holding
    all 600 computation graphs in GPU memory simultaneously.
    Scoring is restricted to the top-num_candidates items by relevance for speed.
    """
    if device is None:
        device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    ...
    for it in range(num_iter):
        ...
        group_slates, group_old_log_probs, group_chosen_local, group_rewards = [], [], [], []
        for g in range(G):
            slates, lps, _, chosen_locals = _collect_trajectories(
                policy_net, user_initial_budget,
                rel_cand, cost_cand, candidate_ids,
        ...
        # GSPO policy update:
        # Per-trajectory backward() with gradient accumulation avoids holding all
        # 600 computation graphs simultaneously (was causing GPU OOM).
        policy_net.train()
        n_traj = G * num_users
        pol_opt.zero_grad()
        for g in range(G):
            for u in range(num_users):
                ...
                # Collect lightweight per-step metadata (no tensors yet)
                step_meta = []
                for k in range(slate_size):
                    cl = group_chosen_local[g][u][k]
                    step_meta.append((k, budget, cum_rel, list(ex_local), cl))
                    ex_local.append(cl)
                    budget  -= float(cost_cand[cl])
                    cum_rel += float(np.exp(rel_cand[cl]))

                # Accumulate sequence-level log ratio (with gradient)
                seq_log_ratio = torch.zeros(1, device=device)
                for k_idx, (k, b, cr, ex_l, cl) in enumerate(step_meta):
                    old_lp = group_old_log_probs[g][u][k_idx]
                    arr    = _build_candidate_features(k, rel_cand, cost_cand, b, cr)
                    new_lp = _action_logprob_over_candidates(
                        policy_net, arr, cl, set(ex_l), device
                    )
                    seq_log_ratio = seq_log_ratio + (new_lp - old_lp)

                # Sequence-level clipped surrogate, normalised by n_traj for mean
                seq_ratio = torch.exp(seq_log_ratio)
                surr1 = seq_ratio * nr
                surr2 = torch.clamp(seq_ratio, 1 - clip_eps, 1 + clip_eps) * nr
                traj_loss = -torch.min(surr1, surr2) / n_traj
                traj_loss.backward()   # accumulate grad, free this trajectory's graph
